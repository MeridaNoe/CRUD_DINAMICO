<template>
  <div>
    ...
    <div class="container mt-5" ref="movieContainer" @scroll="loadMoreMovies" @drop="handleDrop" @dragover.prevent>
...
export default {
  ...
  methods: {
    ...
    async loadMoreMovies() {
      ...
      if (movieContainer.scrollTop + movieContainer.clientHeight >= movieContainer.scrollHeight) {
    ...
    handleDrop: async function (event) {
      ...
      this.dropActivated = true;
      const files = event.dataTransfer.files;
      for (const file of files) {
        console.log('Archivo cargado a la vista:', file.name);
      }
    }
